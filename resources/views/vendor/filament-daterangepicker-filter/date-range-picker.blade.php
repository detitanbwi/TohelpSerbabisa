@php
    ...
    // If the package set the calendar icon as a 'clear' action,
    // convert it to a suffix icon so clicking it opens the date picker instead of clearing!
    if (blank($suffixIcon)) {
        foreach ($suffixActions as $key => $action) {
            if ($action->getName() === 'clear') {
                $suffixIcon = $action->getIcon() ?? 'heroicon-m-calendar-days';
                unset($suffixActions[$key]);
                break;
            }
        }
    }
...
>
    <div
        ...
        x-on:keydown.esc="typeof isOpen === 'function' ? (isOpen() && $event.stopPropagation()) : null"
        x-on:click="if ($event.target.closest('.fi-input-wrp-suffix, .fi-input-wrp-prefix')) { $refs.daterange && ($refs.daterange.focus(), $refs.daterange.click()); }"

        {{ $attributes->merge($getExtraAttributes() , escape: false)->class(['filament-forms-date-time-picker-component fi-fo-date-range-picker-wrp relative']) }}
        {{ $getExtraAlpineAttributeBag() }}
    >
        <style>
            .fi-fo-date-range-picker-wrp .fi-input-wrp-suffix,
            .fi-fo-date-range-picker-wrp .fi-input-wrp-prefix {
                cursor: pointer;
            }
            .fi-fo-date-range-picker-wrp input {
